...
while i_break<max_i
    ...
    if sign(y1)==-sign(y2)
        ...
        fprintf('========OPTION1  number=%d\n',cOp1);
        fprintf('OPTION2  number=%d\n',cOp2);
        fprintf('OPTION3  number=%d\n',cOp3);
        ...
        omegafine        
        nn1 = -KP
        nn2 = polyval(f1,omegafine)
        nn3 = sin(omegafine*L)
        nn4 = polyval(f2,omegafine)
        nn5 = cos(omegafine*L)
        nn6 = polyval(fn,omegafine)
       ...
       % fprintf('%d  != %d \n',a,b);
        ...
        while sign(-KP+(polyval(f1,omegafine)*sin(omegafine*L)+polyval(f2,omegafine)*cos(omegafine*L))/polyval(fn,omegafine))~=sign(y2)
            ...
            %fprintf('omegaFine = omegaFine+stepFine \n');
            %fprintf('%d = %d + %d \n',omegafine,omegafine,stepfine);
        ...
        n1 = -(bn/am)*limfac*omega0(end)^(l-1);
        n2 = 0;
        n3 = 0;
...
%         fprintf('------------------------------------------\n');
        ...
        if mod(l,2)==0
            ...
            n2 = n1*sin(omega0(end)*L);
            n3 = f - n2;
            n4 = abs(n3);
%             fprintf('DELTA ==IF==%d\n',delta);
        else
            delta=abs(f-(-(bn/am)*limfac*omega0(end)^(l-1)*cos(omega0(end)*L)));
            n2 = n1*cos(omega0(end)*L);           
            n3 = f - n2;
            n4 = abs(n3);
%             fprintf('DELTA ==ELSE==%d\n',delta);
        end
...
fprintf('OMEGA0 VECTOR==============================\n');
omega0

fprintf('OMEGAPlus VECTOR===========================\n');
omegaplus=omegaplus(1:jp-1)

fprintf('OMEGAMinus VECTOR==========================\n');
